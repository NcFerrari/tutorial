...
/*******************************************************************************
 * Trida UFO_RUP obsahuje vzorove reseni pro tridu UFO, ktera
 ...
 */
public class RotUFO_RUP extends UFO //_RUP
{
...
//== KONSTRUKTORY A TOVARNI METODY =============================================
     ...
     */
    public RotUFO_RUP(Talir talir, int poradi)
    {
        super( talir, poradi );
        xPos = talir.getX();
        yPos = talir.getY();
...
//== PRISTUPOVE METODY VLASTNOSTI INSTANCI =====================================
     ...
     * @param x  Pozadovana velikost rychlosti ve svislem smeru
...
//    }
    ...
    public String toString()
    {
        return "Cislo_" + poradi + ", x=" + (int)xPos + ", y=" + (int)yPos +
               ", xr=" + (int)xRychlost + ", yr=" + (int)yRychlost +
               ", xt=" + (int)xTah + ", yt=" + yTah;
    }
...
//== NOVE ZAVEDENE METODY INSTANCI =============================================
     ...
     *
     * @param frekvence  Pocet prekresleni vesmiru za sekundu.
     *
     * @throws RunTimeException  Prekroci-li rychlost nektereho UFO
     *                           maximalne povleny limit.
     */
    public void popojed( int frekvence )
    {
        super.popojed( frekvence );
        //System.out.println( "Pred: " + this );
        if( Math.abs(xRychlost) > VSIRKA/3 )
        {
            System.out.println("Standard - prijde vyjimka" );
            System.err.println("CHYBY - prijde vyjimka" );
            throw new RuntimeException( "Prilis velka vodorna rychlost");
            //Odkomentujete-li nasledujici prikazy, vyvolate chybu prekladu,
            //protoze prekladac pozna, ze na ne nikdy nemze dojit.
            //System.out.println("Standard - prisla vyjimka" );
            //System.out.println("Standard - prisla vyjimka" );
        }
        if( Math.abs(yRychlost) > VVYSKA/3 )
            throw new RuntimeException( "Prilis velka vodorna rychlost");
        if(  xPos  < -TV )            xPos = VSIRKA + TV;
        if( xPos  > (VSIRKA + TV) )   xPos = -TV;
        if( yPos  < -TV )             yPos = VVYSKA + TV;
        if( yPos  > (VVYSKA + TV) )   yPos = -TV;
        //System.out.println( "  Po: " + this );
...
//== TESTY A METODA MAIN =======================================================
    ...
    {
        Dispecer.getDispecer().pouzijTridu( RotUFO_RUP.class );
    }


}//public class UFO_RUP extends UFO
