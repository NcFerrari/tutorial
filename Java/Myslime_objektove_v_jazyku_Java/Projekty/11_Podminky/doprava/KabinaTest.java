...
//== SOUKROME A POMOCNE METODY INSTANCI ========================================
    ...
    private TKabina[] vytvorTrojici( Class trida )
    {
        ...
        {
            trojice[i++] = new TKabina( vnejsi,  Smer.VPRED );
            trojice[i++] = new TKabina( stredni, Smer.ZPET  );
            trojice[i++] = new TKabina( vnitrni, Smer.VPRED );
//             trojice[i++] = new TKabina( vnejsi,  Smer.ZPET  );
//             trojice[i++] = new TKabina( stredni, Smer.VPRED );
//             trojice[i++] = new TKabina( vnitrni, Smer.ZPET  );
            P.cekej( 1000 );
        };
        return trojice;
    }
...
//== VNORENE A VNITRNI TRIDY ===================================================
    ...
    private static class TKabina
    {
        ...
        TKabina( Linka linka, Smer smer )
        {
            ...
            try
            {
                kabina = tovarna.invoke( null, 
                                         new Object[] { linka, smer } );
            }
            catch( Exception e )
            {
                throw new RuntimeException(
...
//== VLASTNI TESTY =============================================================
     ...
     * a po potvrzeni od uzivatele pohyb kabin zrychli.
     */
    public void testObaSmery_11a()
    {
        TKabina[] k3 = vytvorTrojici( ObousmernaKabina_11a.class );
        zrychliLinky();
        ukonciSkupinu( k3 );
        zrusLinky();
    }
    ...
    {
        TKabina[] k3 = vytvorTrojici( ObousmernaKabina.class );
        zrychliLinky();
        ukonciSkupinu( k3 );
        zrusLinky();
    }
